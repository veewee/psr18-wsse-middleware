...
### Talk to a peer that puts cipher bytes in MIME parts

New capability, so there is nothing to migrate, but read this if your peer runs Apache CXF with MTOM enabled,
or is .NET/WCF or Metro. Those stacks put the bytes of an `xenc:CipherValue` in a MIME part and leave an
`xop:Include` behind, and they do it **by default**: CXF whenever MTOM is on, .NET and Metro to any large
encrypted content. Nothing negotiates it, and until now such a message was refused as a cipher value that
would not decode.

Put one new block first in your inbound list:
...
$inbound = [
    new Inbound\ResolveOptimizedBytes(
        AttachmentParts::response($attachments, ExternalPartCoverage::Content),
    ),
    new Inbound\Decrypt($privateKey),
    // ... the blocks you already had
];
```

It requires nothing of the message: a peer whose values are all inline passes straight through. To emit the
same shape, `Outbound\Encryption::withOptimizedCipherBytes()` does it, and it cannot be combined with
encrypt-then-sign.

### One thing that may now be refused where it was not

If you sign an element whose content is an `xop:Include`, and you do not register the attachment that pointer
names, signing is now refused and so is verifying such a signature inbound. The signature covered the pointer
while the file travelled unprotected, and the message still satisfied a policy check for that element being
signed.

The fix is one line: register the attachment on the same block.
...
If you already registered your attachments, which is what the attachment-security documentation tells you to
