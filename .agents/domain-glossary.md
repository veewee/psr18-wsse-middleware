...
| **Session key** | A symmetric key (raw bytes) that encrypts the message data, carried wrapped inside an `xenc:EncryptedKey` | Symmetric and per-message; distinct from the RSA Private key that wraps it | `SessionKey` (`src/KeyStore/SessionKey.php:9`) | symmetric key (bare), data key |
...
| **Canonical header block** | The digest form of a header block: the five profile headers, normalized, ascending by name, one CRLF each and no blank line | What both stacks must agree on byte for byte; distinct from the header set an attachment carries, which is unnormalized | `MimeHeaderBlock::canonicalize()` | canonicalized headers, header canonicalization (as the artifact) |
...
| **Content transform** | The signature transform declaring that a reference's digest covers an attachment's content and none of its MIME headers | The signing twin of the Ciphertext transform. Its Complete counterpart also covers the canonical header block, and a bare `sp:Attachments` policy means that one | `Attachment-Content-Signature-Transform` (WSS SwA Profile 1.1) | attachment transform, body transform |
