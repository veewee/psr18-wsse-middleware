...
| **Signing key** | How a Signature block is keyed and how its `ds:KeyInfo` points at that key: a certificate-and-private-key identity, or a symmetric secret | The credential the block takes; distinct from the Private key inside it and from the Key reference it resolves | `SigningKey` (`src/WSSecurity/Outbound/SigningKey.php`) | credential (bare), signer |
