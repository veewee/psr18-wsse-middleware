...
final class SignatureTest extends OutboundTestCase
{
    ...
    public function test_it_lowers_registered_attachments_into_the_signing_request(): void
    {
        ...
        (new Signature(new Asymmetric($this->clientCertificate())))
            ->withSigner($signer)
    ...
    {
        $signer = new RecordingSigner();
        (new Signature(new Asymmetric($this->clientCertificate())))->withSigner($signer)($this->signableContext());
    ...
    public function test_a_context_profile_overrides_the_default(): void
    {
        ...
        (new Signature(new Asymmetric($this->clientCertificate())))->withSigner($signer)($this->context($this->signableEnvelope(), $profile));
    ...
    {
        $signer = new RecordingSigner();
        $block = (new Signature(new Asymmetric($this->clientCertificate())))->withSigner($signer)->withSignatureMethod(SignatureMethod::RSA_SHA1);
    ...
    {
        $signer = new RecordingSigner();
        $block = (new Signature(new Asymmetric($this->clientCertificate())))->withSigner($signer)->withInclusivePrefixes();
        $block($this->signableContext());
    ...
    public function test_with_methods_are_immutable(): void
    {
        ...
        $original = (new Signature(new Asymmetric($certificate)))->withSigner(new RecordingSigner());
    ...
    {
        $signer = new RecordingSigner();
        $block = (new Signature(new Asymmetric($this->clientCertificate())))->withSigner($signer)->withParts([Part::body()]);
        $block($this->signableContext());
    ...
    public function test_direct_reference_embeds_a_bst_and_wires_the_key_info(): void
    {
        ...
        (new Signature(new Asymmetric($this->clientCertificate(), KeyRef::BinarySecurityToken)))->withSigner($signer)($this->context($document));
    ...
    public function test_a_certificate_path_is_advertised_as_a_pkipath_token(): void
    {
        ...
        $block = (new Signature(new Asymmetric(
        ...
        $keyIdentifier = $signer->lastRequest()->keyIdentifier;
        static::assertInstanceOf(DirectReferenceKeyIdentifier::class, $keyIdentifier);
        $keyInfo = $document->stringifyNode($keyIdentifier->apply($document));
    ...
    public function test_it_refuses_an_empty_part_list(): void
    {
        ...
        $block = new Signature(new Asymmetric($this->clientCertificate()));
    ...
    public function test_subject_key_identifier_embeds_no_bst(): void
    {
        $signer = new RecordingSigner();
        $document = $this->signableEnvelope();
        (new Signature(new Asymmetric($this->clientCertificate(), KeyRef::SubjectKeyIdentifier)))->withSigner($signer)($this->context($document));
    ...
    public function test_direct_reference_wires_the_signature_key_info_to_the_embedded_bst(): void
    {
        ...
        (new Signature(new Asymmetric($certificate, KeyRef::BinarySecurityToken)))->withSigner($this->realSigner())($this->context($document));
    ...
    public function test_an_overridden_algorithm_reaches_the_signed_document(): void
    {
        ...
        $block = (new Signature(new Asymmetric($certificate, KeyRef::BinarySecurityToken)))
